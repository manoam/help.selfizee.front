  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-6 py-6 md:py-8">
      <nav className="flex items-center gap-1.5 text-xs text-[var(--a-text-muted)] mb-6">
        ...
        <span className="text-[var(--a-text)] font-medium">Recherche</span>
      </nav>

      <h1 className="text-2xl md:text-3xl font-bold text-[var(--a-text)] mb-6">
        Rechercher
      </h1>

      <div className="relative mb-6">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--a-text-muted)]" />
        <input
          autoFocus
          className="w-full pl-11 pr-4 py-3 text-base bg-white border border-[var(--a-surface-2)] rounded-xl shadow-sm focus:outline-none focus:border-[var(--a-accent)] focus:ring-2 focus:ring-[var(--a-accent)]/20"
      ...
      {isLoading && debouncedQ.trim().length >= 2 && (
        <p className="text-sm text-[var(--a-text-muted)]">Recherche…</p>
      )}
        ...
        <>
          ...
          <div className="space-y-2">
            {results.map((p) => (
              ...
              >
                <FileText className="h-4 w-4 text-[var(--a-text-muted)] mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-[var(--a-text)] group-hover:text-[var(--a-accent)] transition flex items-center gap-2">
                    {p.titre}
                    {p.isFavourite && (
