...
export function ListeDocumentsPage({ source }: { source: Source }) {
  ...
  if (!parent) {
    return (
      <div className="mx-auto max-w-5xl px-4 md:px-6 py-8">
        <p className="text-sm text-[var(--a-text-muted)]">Section introuvable.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-6 py-6 md:py-8">
      <nav className="flex items-center gap-1.5 text-xs text-[var(--a-text-muted)] mb-6 flex-wrap">
        {breadcrumbItems.map((it, idx) => (
          <span key={idx} className="inline-flex items-center gap-1.5">
            ...
            {it.to ? (
              ...
              <span className="text-[var(--a-text)] font-medium">{it.label}</span>
            )}
          </span>
        ))}
      </nav>

      <div className="bg-[var(--a-surface-2)] rounded-xl p-5 md:p-6 mb-6 flex items-start gap-4">
        <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white shrink-0">
          <HeaderIcon className="h-6 w-6 text-[var(--a-accent)]" />
        </span>
