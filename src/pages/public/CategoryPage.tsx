...
export function CategoryPage() {
  ...
  if (!category) {
    return (
      <div className="mx-auto max-w-5xl px-4 md:px-6 py-8">
        <p className="text-sm text-[var(--a-text-muted)]">Catégorie introuvable.</p>
        <Link
          to="/"
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-6 py-6 md:py-8">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-[var(--a-text-muted)] mb-6">
        ...
        <span className="text-[var(--a-text)] font-medium">{category.nom}</span>
      </nav>
          ...
          {category.subCategories.map((sub) => (
            <div key={sub.id} className="a-card p-5">
              <Link
                ...
                className="text-base font-semibold text-[var(--a-text)] mb-1 hover:text-[var(--a-accent)] transition inline-block"
              >
                {sub.nom}
                {sub._count?.posts !== undefined &&
                  sub._count.posts > 0 && (
                    <span className="ml-2 text-xs text-[var(--a-text-muted)] font-normal">
                      ({sub._count.posts})
                    </span>
                  )}
              </Link>
      ...
      {posts.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-[var(--a-text)] mb-3 uppercase tracking-wide">
            Articles
          </h2>
            ...
            {posts.map((p) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {p.resume && (
                    <p className="text-xs text-[var(--a-text-muted)] mt-0.5 line-clamp-1">
                      {p.resume}
                    </p>
                  )}
                </div>
